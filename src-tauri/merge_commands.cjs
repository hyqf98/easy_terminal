...
const filePath = path.join(__dirname, 'commands/system/linux.json');

// Read existing file
const data = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
const existingNames = new Set(data.commands.map(cmd => cmd.name));
console.log('Existing command count:', existingNames.size);
...
if (duplicates.length > 0) {
  console.log('Duplicate commands (will be skipped):');
...
console.log();
console.log('New commands to add:', toAdd.length);
console.log('Duplicates skipped:', duplicates.length);

// Add new commands
data.commands.push(...toAdd);
...
console.log();
console.log('Total commands after merge:', data.commands.length);
